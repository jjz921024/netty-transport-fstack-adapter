...
public final class Native {
    ...
    // -Ddpdk.libs=rte_common_cpt,rte_common_dpaax
    private static final String DPDK_LIBS = SystemPropertyUtil.get("dpdk.libs", "");
    ...
    // isSupportingUdpSegment() 需要在ff_init前创建socket, 暂时屏蔽
    static final boolean IS_SUPPORTING_UDP_SEGMENT = false;
    ...
    static long epollWait(FileDescriptor epollFd, EpollEventArray events, FileDescriptor timerFd,
        ...
        // 不支持纳秒级别超时
        /*int millis = (int) (Math.min(timeoutSec * 1000000000L + timeoutNs, millisThreshold * 1000000L) / 1000000L);
        int result = epollWait(epollFd.intValue(), events.memoryAddress(), events.length(), millis);*/
    ...
    public static native int fstackInit(String confPath, int id, String type);

    private static void loadNativeLibrary() {
        for (String libName : DPDK_LIBS.split(",")) {
            try {
                System.loadLibrary(libName);
                System.out.println("load lib: " + libName);
            } catch (UnsatisfiedLinkError e) {
                System.out.println("Failed to load lib" + libName + ", " + e.getMessage());
                throw e;
            }
        }
