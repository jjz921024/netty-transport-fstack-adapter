...
public class EpollEventLoop extends SingleThreadEventLoop {
    ...
    // 不立即唤醒epoll标志位, true时epoll在wait阻塞中
    ...
    EpollEventLoop(EventLoopGroup parent, Executor executor, SelectStrategy strategy,
                   RejectedExecutionHandler rejectedExecutionHandler, EventLoopTaskQueueFactory taskQueueFactory,
                   EventLoopTaskQueueFactory tailTaskQueueFactory,
                   String confPath, int procId, String procType) {
        super(parent, executor, false, newTaskQueue(taskQueueFactory), newTaskQueue(tailTaskQueueFactory),
                rejectedExecutionHandler);
        selectStrategy = ObjectUtil.checkNotNull(strategy, "strategy");
        events = new EpollEventArray(4096);
    ...
    protected void wakeup(boolean inEventLoop) {
        if (!inEventLoop && nextWakeupNanos.getAndSet(AWAKE) != AWAKE) {
            Native.keventTriggerUserEvent(epollFd.intValue(), 0);
        }
    }*/
    ...
    private int epollWait(long deadlineNanos) throws IOException {
        ...
        return Native.epollWait(epollFd, events, delaySeconds, delayNanos, EPOLL_WAIT_MILLIS_THRESHOLD);
    }
    ...
    private void kernelBypassRun() {
        try {
            // 初始化fstack要和run同一线程
            if (Native.fstackInit(confPath, procId, procType) != 0) {
                throw new RuntimeException("fstack init fail");
            }

            // 创建epoll延迟在fstack初始化后
            openFileDescriptors();

            // 调ffRun后线程会阻塞
            fstackRun(epollFd.intValue(), events.memoryAddress(), events.length());
        } finally {
            for (;;) {
                try {
                    if (isShuttingDown()) {
    ...
    public native void fstackRun(int epfd, long address, int len);
