...
int ff_eventfd_write(int fd, eventfd_t *value) {
    return ff_write(fd, value, sizeof(eventfd_t)) != sizeof(eventfd_t) ? -1 : 0;
}
...
static jint netty_epoll_native_JNI_OnLoad(JNIEnv* env, const char* packagePrefix) {
    ...
    if (netty_epoll_fstack_JNI_OnLoad(env) == JNI_ERR) {
        goto done;
    }
...
static void netty_epoll_native_JNI_OnUnload(JNIEnv* env) {
    netty_epoll_fstack_JNI_OnUnload(env);
